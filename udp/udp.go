package udp

import (
	"fmt"
	"net"
	"study/ip"
	"syscall"
)

func Send(address string, port uint16, data []byte) error {
	var dst [4]byte
	copy(dst[:], net.ParseIP(address).To4())

	ih := &ip.Header{
		VersionIHL:         (4 << 4) | (5 & 0x0F),
		Tos:                0,
		TotalLength:        28 + uint16(len(data)),
		Identification:     0,
		FlagsFragment:      0x4000, // df = 1
		TTL:                64,
		Protocol:           17, // udp
		SourceAddress:      [4]byte{127, 0, 0, 1},
		DestinationAddress: dst,
	}
	buf := ih.BuildHeader()

	h := &Header{
		SourcePort:      64152,
		DestinationPort: port,
		Length:          8 + uint16(len(data)),
	}
	buf = append(buf, h.BuildPacket(&PseudoHeader{
		SourceAddress:      ih.SourceAddress,
		DestinationAddress: ih.DestinationAddress,
		Zero:               0,
		Protocol:           ih.Protocol,
		Length:             8 + uint16(len(data)),
	}, data)...)

	s, err := syscall.Socket(syscall.AF_INET, syscall.SOCK_RAW, syscall.IPPROTO_RAW)
	if err != nil {
		return fmt.Errorf("socket errror: %s", err.Error())
	}
	defer syscall.Close(s)

	err = syscall.SetsockoptInt(s, syscall.IPPROTO_IP, syscall.IP_HDRINCL, 1)
	if err != nil {
		return fmt.Errorf("socket option error: %s", err.Error())
	}

	err = syscall.Sendto(s, buf, 0, &syscall.SockaddrInet4{
		Port: 0,
		Addr: dst,
	})
	if err != nil {
		return fmt.Errorf("socket send error: %s", err.Error())
	}

	return nil
}
